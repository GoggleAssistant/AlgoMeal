...
while($row = $res_cat->fetch_assoc()) {
    if (isset($categories[$row['category']])) $categories[$row['category']] += (float)$row['total'];
    else $categories[$row['category']] = (float)$row['total'];
}
...
<style>
    ...
    <style>
        .btn-m3 {
            ...
            padding: 10px 20px; border-radius: 100px; font-weight: 700; font-size: 0.85rem;
        ...
        .mgmt-tabs { display: flex; gap: 0.75rem; margin-bottom: 2rem; padding-bottom: 1rem; border-bottom: 1px dashed var(--border); overflow-x: auto; }
        .tab-btn { padding: 0.6rem 1.5rem; background: var(--surface); border: 1px solid var(--border); border-radius: 100px; color: var(--text-muted); font-weight: 700; font-size: 0.8rem; cursor: pointer; transition: all 0.2s ease; display:flex; align-items:center; justify-content:center; white-space:nowrap; }
        .tab-btn:hover { background: #f8fafc; border-color: #cbd5e1; color: var(--text-main); }
        .tab-btn.active { background: var(--primary); color: white; border-color: var(--primary); box-shadow: 0 4px 12px rgba(0,97,255,0.25); }

        .tab-content { display: none; }
        .tab-content.active { display: block; }

        .doc-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 1.5rem; }
        .doc-card { background: white; border: 1px solid var(--border); border-radius: 12px; overflow: hidden; transition: 0.2s; }
        .doc-card:hover { transform: translateY(-4px); box-shadow: var(--shadow-md); }
        .doc-img { width: 100%; height: 180px; object-fit: cover; background: #f1f5f9; border-bottom: 1px solid var(--border); }
        .doc-meta { padding: 1rem; }
        .doc-date { font-size: 0.7rem; font-weight: 800; color: var(--primary); text-transform: uppercase; margin-bottom: 0.25rem; }
        .doc-desc { font-size: 0.85rem; color: var(--text-main); font-weight: 600; line-height: 1.4; margin-bottom: 0.5rem; }
        .doc-uploader { font-size: 0.7rem; color: var(--text-muted); border-top: 1px solid #f1f5f9; padding-top: 0.5rem; display: flex; align-items: center; gap: 4px; }
        
        .attendance-grid { background: white; overflow-x: auto; border: 1px solid var(--border); border-radius: 12px; }
        .att-table { width: 100%; border-collapse: collapse; font-size: 0.75rem; }
        .att-table th, .att-table td { border: 1px solid var(--border); padding: 0.5rem; text-align: center; }
        .att-table th { background: #f8fafc; font-weight: 800; }
        .att-table td.name { text-align: left; font-weight: 700; background: #fcfcfc; white-space: nowrap; }
        .att-status-s { color: #10b981; font-weight: 900; }
        .att-status-a { color: #f43f5e; font-weight: 900; }

        @media print { 
            .no-print { display: none !important; } 
            .tab-content { display: block !important; } 
            body.print-roster-only * { visibility: hidden; }
            body.print-roster-only .attendance-grid, 
            body.print-roster-only .attendance-grid * { visibility: visible; }
            body.print-roster-only .attendance-grid { position: absolute; left: 0; top: 0; width: 100%; padding:0; margin:0; border:none; }
        }
    </style>
    ...
    <div class="content">
        <div class="mgmt-header no-print">
            ...
            <div style="display:flex; gap:0.5rem;">
                <button class="btn-m3 btn-m3-outline" onclick="window.print()"><span class="material-icons">print</span> Print Page</button>
            </div>
        </div>
        ...
        <div id="tab-fiscal" class="tab-content <?= (!isset($_GET['tab']) || $_GET['tab'] == 'fiscal') && !isset($_GET['page_offset']) ? 'active' : '' ?>">
            <div class="kpi-row no-print">
                ...
                <div class="kpi-card <?= $remaining_funds < 50000 ? 'warning' : '' ?>"><div class="kpi-label">Liquidity Pool</div><div class="kpi-value">&#8369;<?=number_format($remaining_funds,2)?></div></div>
            </div>

            <div class="main-grid">
                <div style="display:flex; flex-direction:column; gap:1.5rem;">
                    <div class="dashboard-card no-print">
                        <div class="card-header"><span class="material-icons">pie_chart</span><h3>Budget Distribution</h3></div>
                        <div style="height: 200px; padding: 1rem;"><canvas id="allocationChart"></canvas></div>
                    </div>
                    <div class="dashboard-card no-print">
                        <div class="card-header"><span class="material-icons">receipt_long</span><h3>Manual Log</h3></div>
                        <?php if($isAdmin): ?>
                        <div class="logging-form">
                            <div class="form-grid">
                                <div class="form-field"><label>Amount</label><input type="number" id="logAmount" placeholder="0.00"></div>
                                <div class="form-field"><label>Category</label><select id="logCategory"><option>Labor Costs</option><option>Logistics</option><option>Misc</option></select></div>
                            </div>
                            <div class="form-field" style="margin-bottom:1rem;"><label>Description</label><input type="text" id="logDesc"></div>
                            <button class="btn-m3 btn-m3-primary" style="width:100%; border-radius: 8px;" onclick="submitLog()">Record Log</button>
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
                <div class="dashboard-card">
                    <div class="card-header"><span class="material-icons">history</span><h3>Expediture Ledger</h3></div>
                    <table class="reports-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><span class="budget-tag tag-<?= strtolower(explode(' ', $r['category'])[0]) ?>"><?= $r['category'] ?></span></td>
            ...
            <div class="doc-grid">
                ...
                <div class="doc-card">
                    ...
                    <div class="doc-meta">
                        ...
                        <div class="doc-uploader"><span class="material-icons" style="font-size:12px;">person</span> Uploader: <?= $d['uploader'] ?? 'Staff' ?></div>
                    </div>
                </div>
                <?php endwhile; if($docs->num_rows === 0): ?>
        ...
        <div id="tab-attendance" class="tab-content <?= (isset($_GET['tab']) && $_GET['tab'] == 'attendance') || isset($_GET['page_offset']) ? 'active' : '' ?>">
            ...
            <div class="no-print" style="margin-bottom:1.5rem;">
                ...
                <p style="margin:0; font-size:0.8rem; color:var(--text-muted);">Comprehensive tracking matrix of all verified meals.</p>
            </div>
            
            <div class="no-print" style="margin-bottom:1.5rem; display:flex; justify-content:flex-end;">
                <button class="btn-m3 btn-m3-outline" onclick="printRosterIsolated()" style="gap:6px; background:white; color:var(--text-main);"><span class="material-icons" style="font-size:18px; color:var(--primary);">print</span> Print Roster</button>
            </div>
            
            <div class="attendance-grid" style="overflow-x:auto; margin-bottom:2rem; border-radius:12px; border:1px solid var(--border);">
                <table class="att-table" style="min-width: 100%;">
                    <thead>
                        <tr>
                            <th style="min-width:200px; border-left:none; vertical-align:bottom; position:sticky; left:0; background:#f8fafc; z-index:2; border-right:2px solid var(--border);">Student Name</th>
                            <?php foreach($days as $d): ?>
                                <th style="vertical-align:bottom; min-width:140px; background:#f8fafc;">
                                    <div style="font-size:0.7rem; font-weight:800; color:var(--primary); margin-bottom:4px;"><?= date('D, M d Y', strtotime($d)) ?></div>
                                    <div style="font-size:0.8rem; font-weight:700; color:var(--text-main); line-height:1.2;"><?= htmlspecialchars($dishes[$d]) ?></div>
                                </th>
                            <?php endforeach; ?>
                        </tr>
                    </thead>
